...
AnnotatedMarcSerializer.bibIndexRules = function (bibRecordIndexRulesContent) {
  return bibRecordIndexRulesContent.split('\n')
    // Match known components:  (037)   (SERIES)((s))   (880..                  ) ..  (690 )    (REM               )   (23468) (Subject)((d))
    .map((line) => line.match(/^(\d+) > (\w+)\((\w)\)\s+((\d|\.|\[[\d ]+\]){3,5})(\.\.(\w+))?\s+(KEEP|REM|EACH|N\/A)\s+(\w+)\s+([^[]+\(\w\))/))
    .filter((m) => m)
    .map((matches) => {
      return {
        indexName: matches[2],
  ...
  return bib.varFields
    .filter((field) => {
      ...
      return rule.marcIndicatorRegExp.test(fieldMarcIndicator) &&
        rule.secondaryMarcIndicatorRegExp.test(fieldMarcIndicator)
    })
}
