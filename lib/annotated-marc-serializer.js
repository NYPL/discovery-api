...
AnnotatedMarcSerializer.formatVarFieldMatch = function (matchingVarField, rule) {
  const extractContentFor1xxOr700 = function (subfield) {
    if (subfield.tag === '4') {
      let newContent = subfield.content
      let words = newContent.split(' ')
      words.forEach((word) => {
        let replacer = word.length === 3 && relatorMappings[word]
        if (replacer) {
  ...
  const extractContent = (/^(1..|700|710|711|791)/.test(matchingVarField.marcTag) ? extractContentFor1xxOr700 : (subfield) => subfield.content)
  ...
  const content = matchingVarField.content || matchedSubfields.map(extractContent).join(' ')
