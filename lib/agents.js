...
module.exports = function (app) {
  ...
  var getAgentById = function (id, cb) {
    app.esClient.get({
      index: 'agents',
      type: 'agent',
      id: id
    }).then((resp) => {
      cb(serializers.AgentSerializer.serialize(resp._source))
    }, function (err) {
  ...
  app.agents.findById = function (params, cb) {
    app.db.returnCollectionTripleStore('agents', function (err, agents) {
      if (err) throw err
      agents.find({ uri: parseInt(params.id) }).toArray(function (err, results) {
        if (err) throw err
        if (results.length === 0) {
          cb(false)
        } else {
          cb(serializers.ResourceSerializer.serialize(resp._source))
