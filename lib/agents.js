...
//PARSING
	...
	var convertAgentLookupToJsonLd = function(rawAgentData){
		...
		if (rawAgentData['rdf:type']) rawAgentData['rdf:type'].forEach(type =>{
			base['@type'].push(type.objectUri)
		})

		if (rawAgentData.uri) base['@id'] = "agents:"+rawAgentData.uri

		if (rawAgentData['skos:exactMatch']) rawAgentData['skos:exactMatch'].forEach(value =>{
		...
		if (rawAgentData['skos:prefLabel']) rawAgentData['skos:prefLabel'].forEach(value =>{
			base.prefLabel.push(value.objectLiteral)
		})
		if (rawAgentData['dbo:birthDate']) rawAgentData['dbo:birthDate'].forEach(value =>{
			base.birthDate.push(value.objectLiteral)
		})
		if (rawAgentData['dbo:deathDate']) rawAgentData['dbo:deathDate'].forEach(value =>{
			if (base.birthDate.indexOf(value.objectLiteral) == -1){
				base.birthDate.push(value.objectLiteral)
			}
		})
		if (rawAgentData['foaf:isPrimaryTopicOf']) rawAgentData['foaf:isPrimaryTopicOf'].forEach(value =>{
			base.wikipedia.push(value.objectLiteral)
		})
		if (rawAgentData['foaf:depiction']) rawAgentData['foaf:depiction'].forEach(value =>{
			base.depiction.push(value.objectLiteral)
		})
		if (rawAgentData['dcterms:description']) rawAgentData['dcterms:description'].forEach(value =>{
		...
		images.forEach(i =>{
			base["itemListElement"].push({
				...
				"result": {
					...
					filename: i.imageId.map(img =>  img )
				}
			})
		})
		return base
