...
clientWrapper.recapCustomerCodeByBarcode = (barcode) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    timeoutId = setTimeout(() => reject(new Error(`Exhausted ${timeoutMs}ms timeout waiting for SCSB`)), timeoutMs)
  })
...
clientWrapper.__recapCustomerCodesByBarcodes = async function (barcodes) {
  ...
  return Promise.all(
    barcodes.map(clientWrapper.recapCustomerCodeByBarcode)
  ).then((scsbResponses) => {
