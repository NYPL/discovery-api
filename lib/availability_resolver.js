...
class AvailabilityResolver {
  ...
  responseWithUpdatedAvailability (request, itemUri) {
    return this._createSCSBBarcodeAvailbilityMapping(this.barcodes)
      .then((barcodesAndAvailability) => {
        this._fixItemAvailabilityInResponse(barcodesAndAvailability, request, itemUri)
  ...
  _fixItemAvailabilityInResponse (barcodesAndAvailability, request, itemUri) {
    for (let hit of this.elasticSearchResponse.hits.hits) {
      (hit._source.items || []).map((item) => {
        ...
        if (isInRecap) {
          ...
          if (itemUri){
            const mostRecentCustomerCode = await this.SCSBRestClient.search({ fieldValue: barcode, fieldName: 'Barcode' })
