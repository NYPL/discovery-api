...
let requestableBasedOnHoldingLocation = require('./requestability_determination').requestableBasedOnHoldingLocation
...
class AvailabilityResolver {
  ...
  _checkScsbForRecapCustomerCode () {
    ...
    return this.restClient.recapCustomerCodeByBarcode(barcode)
  ...
  _fixItemRequestability () {
    for (let hit of this.elasticSearchResponse.hits.hits) {
      (hit._source.items || []).map((item) => {
        ...
        item.eddRequestable[0] = this._fixEddRequestability(item, isInRecap)
        item.physRequestable[0] = this._fixPhysRequestability(item, isInRecap)
        item.specRequestable[0] = !!item.aeonUrl
        return item
      })
    }
  }
  ...
  _fixPhysRequestability (item, isInRecap) {
    ...
    // open_locations.
    if (isInRecap) {
      return DeliveryLocationsResolver.__deliveryLocationsByCustomerCode.length > 0
    } else {
      return requestableBasedOnHoldingLocation(item)
    }
  }
  ...
  //               item.status[0].id = config.get('itemAvailability.available.id')
  //               item.status[0].label = config.get('itemAvailability.available.label')
  //               item.physRequestable = requestableBasedOnHoldingLocation(item)
  ...
  //               if (item.recapCustomerCode && item.recapCustomerCode[0] && recapCustomerCodes[item.recapCustomerCode[0]]) {
  //                 item.eddRequestable = recapCustomerCodes[item.recapCustomerCode[0]].eddRequestable
  //               }
  //               item.status[0].id = config.get('itemAvailability.available.id')
  //               item.status[0].label = config.get('itemAvailability.available.label')
  //             } else {
