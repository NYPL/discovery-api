...
const requestableBasedOnHoldingLocation = require('./requestability_determination').requestableBasedOnHoldingLocation
  ...
  _fixPhysRequestability (item, isInRecap) {
    ...
    if (isInRecap) {
      ...
      if (item.recapCustomerCode && item.recapCustomerCode[0]) {
        const deliveryLocations = DeliveryLocationsResolver.__deliveryLocationsByCustomerCode(item.recapCustomerCode)
      ...
      if (item.m2CustomerCode && item.m2CustomerCode[0]) {
        deliveryLocations = DeliveryLocationsResolver.__deliveryLocationsByCustomerCode(item.m2CustomerCode)
      }
      const requestableByHoldingLocation = requestableBasedOnHoldingLocation(item)
      return requestableByHoldingLocation && deliveryLocations && deliveryLocations.length
    }
  }
