...
class AvailabilityResolver {
  constructor(responseReceived) {
    ...
    this.ScsbClient = new ScsbClientWithRecap({ url: process.env.SCSB_URL, apiKey: process.env.SCSB_API_KEY }
    )
  }
  ...
  _createSCSBBarcodeAvailbilityMapping(barcodes) {
    ...
    return this.ScsbClient.getItemsAvailabilityForBarcodes(this.barcodes)
      .then((itemsStatus) => {
  ...
  _fixItemRequestabilityInResponse(barcodesAndAvailability, request, options) {
    for (let hit of this.elasticSearchResponse.hits.hits) {
      (hit._source.items || []).map((item) => {
        ...
        if (isInRecap) {
          ...
          if (options.queryRecapCustomerCode) {
            this.ScsbClient.recapCustomerCodeByBarcode(barcode)
