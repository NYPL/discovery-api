...
module.exports = function (app, _private = null) {
  ...
  app.contributors.browse = function (params, opts, request) {
    ...
    app.logger.info('Contrbutors#browse', BROWSE_INDEX, body)
    const splain = app.esClient.explain({query:body.query, id: "contributor_itinga, yitshak aharon, ha-levi, 1827-1891", index: 'browse-prod' })
    console.dir(splain, {depth: null})
  ...
  if (_private && typeof _private === 'object') {
    ...
    _private.buildElasticRoleCountQuery = buildElasticRoleCountQuery
  }
}
    ...
    query: {
      bool: {
        should: [
          {
            terms: {
              'creatorLiteral.keywordLowercasedStripped': contributorList
            }
          },
          {
            terms: {
              'contributorLiteral.keywordLowercasedStripped': contributorList
            }
          }
        ],
