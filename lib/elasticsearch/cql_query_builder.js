...
function buildEsQueryFromTree (tree) {
  switch (tree.type) {
    case 'query':
      queries = tree.children.filter(child => child.type.includes('query'))
 ...
 */
function atomicQueryParams (atomic_query) {
  return {
    scope: atomic_query.children.find(child => child.type === 'scope').text.trim(),
    relation: atomic_query.children.find(child => child.type === 'relation').text.trim(),
    term: findTopPhrase(atomic_query),
    terms: findTopWords(atomic_query)
  }
}
