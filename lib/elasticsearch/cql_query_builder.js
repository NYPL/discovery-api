...
const controlledVocabularies = require('../models/ControlledVocabularies').getData()
...
function buildAtomicMain ({ fields, relation, terms, term, scope }) {
  return !Object.keys(controlledVocabularies).includes(scope)
    ? buildAtomicQueryByRelation({ fields, relation, terms, term })
    : mappedQueryForControlledVocabulary({ fields, relation, terms, term, scope })
}

function mappedQueryForControlledVocabulary ({ fields, relation, terms, term, scope }) {
  ...
  const controlledVocabFields = controlledVocabularies[scope]
