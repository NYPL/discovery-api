...
const { indexMapping, NYQL_BOOSTING } = require('./cql/index-mapping')
...
class CqlQuery {
  ...
  buildEsQuery (request = null, options = {}) {
    const filterQuery = buildFilterQuery(request, options)
    return {
      bool: {
        must: [
...
function buildFilterQuery (request, options) {
  if (!request) return {}
  const queryJson = ElasticQueryBuilder.forApiRequest(request, options).query.toJson()
...
function applyBoosting (field, scope) {
  const fieldStr = typeof field === 'string' ? field : field.field
  const [baseField] = fieldStr.split('^')
  if (NYQL_BOOSTING && NYQL_BOOSTING[scope] && NYQL_BOOSTING[scope][baseField]) {
    return `${baseField}^${NYQL_BOOSTING[scope][baseField]}`
  }
  return fieldStr
}
...
function buildAtomic ({ scope, relation, terms, term }) {
  ...
  const allFields = nestedFilterAndMap(
    ...
    field => applyBoosting(field, scope)
  )
...
function matchTermWithFields (fields, term, type) {
  ...
  // if config includes exact match boosting, exactBoostingSelector = selectFields('exact', fields)
  // if config includes prefix match boosting and queryType isn't exact, prefixBoostingSelector = selectFields('prefix', fields)
  ...
  const queries = [
    ...
    // if defined, add exact match boosting queries and prefix match boosting queries
  ]
...
// For a given date string, return the next date that would be considered outside
// of the range of that date string.
// We want to do this so that e.g. > 1999 means greater than or equal to Jan 1 2000,
// rather than greater than Jan 1 1999, which would include all of 1999.
...
function nextDate (dateString) {
  ...
  // detect how specific the date string is,
  // and increment to the next date fully outside the user-specified range
  if (match[3]) {
