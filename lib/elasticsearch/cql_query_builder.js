...
function buildEsQuery (cqlQuery, request) {
  const filterQuery = filterQuery(request)
  return {
    query: {
      bool: {
        should: [
...
function dateQueries ({ fields, relation, terms, term }) {
  if (!fields.some(field => field.includes('date'))) { return [] }
  let range
  switch (relation) {
    case "<":
      range = { lt: terms[0] }
