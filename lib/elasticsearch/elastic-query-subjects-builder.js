...
class ElasticQuerySubjectsBuilder {
  ...
  */
  requireContainingMatch () {
    const words = this.request.querySansQuotes().split(/\s+/).filter(Boolean)
    const multiPrefix = []

    // build a bool query that supports prefixes for each word while still requiring the remaining words
    words.forEach(word => {
      const otherWords = words.filter(w => w !== word).join(' ')

      const mustClause = []
      mustClause.push({ prefix: { preferredTerm: { value: word } } })
      if (otherWords) {
        mustClause.push({ match: { preferredTerm: { query: otherWords, operator: 'and' } } })
      }

      multiPrefix.push({ bool: { must: mustClause } })
    })
    ...
    this.query.addMust({
      bool: {
        should: [
          ...
          { bool: { should: multiPrefix, _name: 'preferredTermPrefix' } },
