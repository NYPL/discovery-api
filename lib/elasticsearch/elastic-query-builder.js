  ...
  */
  ...
  filterClausesForDateParams (filters) {
    ...
    if (dateBefore) {
      ...
      if (!dateBefore.match(dateRegex)) { dateBefore = null }
    }
    if (dateAfter) {
      dateAfter = dateAfter.toString()
      if (!dateAfter.match(dateRegex)) { dateAfter = null }
    }
    ...
    if (dateBefore) {
      ...
      if (dateMatch && dateMatch[5]) {
        range.lte = `${dateMatch[1]}-${dateMatch[3]}-${dateMatch[5]}T23:59:59`
      } else if (dateMatch && dateMatch[3] && parseInt(dateMatch[3]) < 12) {
        range.lt = `${dateMatch[1]}-${parseInt(dateMatch[3]) + 1}`
      } else if (dateMatch) {
