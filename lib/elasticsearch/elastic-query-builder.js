...
const { buildPackedTerms } = require('./utils')
  ...
  */
  applyFilters () {
    ...
    filterClausesWithPaths = filterClausesWithPaths.concat(simpleMatchFilters.map((prop) => {
      ...
      const buildClause = (value, field) => {
        const fieldIsPackedField = field.match(/_packed$/)
        const valueIsNotPackedValue = value.indexOf('||') < 0
        if (valueIsNotPackedValue && fieldIsPackedField) {
          return buildPackedTerms(field, value)
        } else return { term: { [field]: value } }
      }
      const clausesForEachField = (value) => config.field.map((field) => buildClause(value, field))
      ...
      const clause = (Array.isArray(value)) ? { bool: { [booleanOperator]: value.map(clausesForEachField) } } : clausesForEachField(value)
      return { path: config.path, clause }
    }))

    // Gather root (not nested) :
