...
class ElasticQueryBuilder {
  ...
  */
  ...
  applySubjectPrefix () {
    ...
    this.query.addMust({
      bool: {
        should: [
          prefixMatch('subjectLiteral.keywordLowercasedStripped', q),
          prefixMatch('parallelSubjectLiteral.keywordLowercasedStripped', q)
        ]
      }
    })
    this.query.addShoulds([
      termMatch('subjectLiteral.keywordLowercasedStripped', q, 50),
      termMatch('parallelSubjectLiteral.keywordLowercasedStripped', q, 50)
    ])
  }
  ...
  */
  ...
  requireSpecificIdentifierMatch () {
    if (this.request.params.lccn) {
      ...
      this.query.addMust({ term: { 'idOclc.clean': this.request.params.oclc } })
    }
  }
  ...
  **/
  boostSubjectMatches () {
    const q = this.request.querySansQuotes()
    this.query.addShoulds([
      phraseMatch('subjectLiteral.keywordLowercasedStripped', q, 50),
      phraseMatch('parallelSubjectLiteral.keywordLowercasedStripped', q, 50),
      prefixMatch('subjectLiteral.keywordLowercasedStripped', q, 50),
      prefixMatch('parallelSubjectLiteral.keywordLowercasedStripped', q, 50),
      termMatch('subjectLiteral.keywordLowercasedStripped', q, 50),
      termMatch('parallelSubjectLiteral.keywordLowercasedStripped', q, 50)
    ])
  }
  ...
  */
  requireMultiMatch (fields, value = this.request.params.q) {
    ...
    if (!/(OR|AND|")/.test(value)) { return }

    const filterShould = []

    filterShould.push({
      query_string: namedQuery({
        fields: fieldMap._root,
        query: escapeQuery(value),
        type: 'cross_fields',
        // For 4+ term queries, require 75% of terms:
    ...
    this.query.addFilter(filterShould.length === 1 ? filterShould[0] : { bool: { should } })
  }
  ...
  **/
  ...
  buildMultiFieldClause (value, fields) {
    return {
      bool:
        { should: fields.map(field => this.buildFilterClause(value, [field])) }
    }
  }
  ...
  buildFilterClause (value, fieldsToMatchOn) {
    ...
    if (filterMatchesOnMoreThanOneField) {
      ...
      // FIXME: Temporary workaround in lieu of mappings changes:
      // If matching on subjectLiteral, contributorLiteral, of creatorLiteral,
      // additional match on trailing period version of value:
      if (/Literal/.test(field) && !/\.$/.test(value)) {
  ...
  */
  ...
  filterClausesForDateParams (filters) {
    let dateBefore = (filters.dateBefore || filters.dateTo)
    let dateAfter = (filters.dateAfter || filters.dateFrom)

    const dateRegex = /(\d{4})(?:[-/](\d{2}))?(?:[-/](\d{2}))?/
    if (dateBefore) {
      dateBefore = dateBefore.toString()
      if (!dateRegex.test(dateBefore)) { dateBefore = null }
    }
    if (dateAfter) {
      dateAfter = dateAfter.toString()
      if (!dateRegex.test(dateAfter)) { dateAfter = null }
    }
    ...
    const range = { relation: 'intersects' }
    if (dateAfter) { range.gte = `${dateAfter.replaceAll('/', '-')}` }
    if (dateBefore) {
      const dateMatch = dateBefore = dateBefore.match(dateRegex)
      if (dateMatch && dateMatch[3]) {
        range.lte = `${dateMatch[1]}-${dateMatch[2]}-${dateMatch[3]}T23:59:59`
      } else if (dateMatch && dateMatch[2] && parseInt(dateMatch[2]) < 12) {
        range.lt = `${dateMatch[1]}-${parseInt(dateMatch[2]) + 1}`
      } else if (dateMatch) {
        range.lt = `${parseInt(dateMatch[1]) + 1}`
      }
    }
