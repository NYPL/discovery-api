  ...
  applyFilters () {
    ...
    filterClausesWithPaths = filterClausesWithPaths.concat(simpleMatchFilters.map((prop) => {
      ...
      const buildClause = (value) => {
        if (config.operator !== 'match') return null
        // If filtering on a packed field and value isn't a packed value:
        if (value.indexOf('||') < 0 && config.field.match(/_packed$/)) {
          return buildPackedTerms(config.field, value)
        } else return { term: { [config.field]: value } }
      }
