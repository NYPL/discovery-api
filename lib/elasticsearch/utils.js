...
*/
const buildEsMatchClause = (matcher, field, value, boost, ignoreCase = false) => {
  let clause = {
    [matcher]: {
      [field]: namedQuery({
        value,
  ...
  if (ignoreCase) {
    clause[matcher][field].case_insensitive = true
  }
  // Nested field?
...
*/
const prefixMatch = (field, value, boost = 1, ignoreCase = false) => {
  return buildEsMatchClause('prefix', field, value, boost, ignoreCase)
}
...
*/
const termMatch = (field, value, boost = 1, ignoreCase = false) => {
  return buildEsMatchClause('term', field, value, boost, ignoreCase)
}
...
*
* For example:
*   const body = { query: wildcardMatch('idBarcode', '6789' }
*/
const wildcardMatch = (field, value, boost = 1, ignoreCase = false) => {
  return buildEsMatchClause('wildcard', field, '*' + value + '*', boost, ignoreCase)
}
