...
module.exports = function(app) {
  ...
  var searchResourcesByFilters = function(params, cb) {
    ...
    if(params.value)
      shoulds.push({
        "multi_match": {
          "query": params.value,
          "fields": ['uri^10','title^10','description^5','termLabels^5','contributorLabels^5','note']
        }
      })
    ...
    app.esClient.search({
      ...
      body: {
       ...
       //  console.log("  processing result: ", resp.hits.hits.length)
        var hits = resp.hits.hits;        
        hits.forEach(function(h){
          results.push(h._source)
        })
  ...
  app.resources.randomResources = function(params, cb) {
    params = util.parseParams(params, { count: { type: 'int', default: 3, range: [1,20] } })
    ...
    app.esClient.search({
      ...
      body: {
        ...
        cb(convertResourceSearchToJsonLd(results))
    }, function (err) {
        //console.trace(err.message);
        cb(false)
    })
