...
module.exports = function (app, _private = null) {
    ...
    if (params.itemUri) {
      ...
      const itemsOptions = {
        ...
        query: {
          ...
          include_item_aggregations: params.include_item_aggregations
        }
      }
  ...
  const addInnerHits = (body, _options = {}) => {
    ...
    // if (!options.merge_checkin_card_items) {
    //   clauses.push({
    //     bool: {
    //       must_not: {
    //         term: {
    ...
    // }
    //
    // // Filtering on item date, volume, etc?
    // if (options.query) {
    //   if (options.query.volume) {
    //     clauses.push({
    //       range: {
    //         'items.volumeRange': esRangeValue(options.query.volume)
    //       }
    //     })
    //   }
    ...
    //     clauses.push({
    //       range: {
    //         'items.dateRange': esRangeValue(options.query.date)
    //       }
    //     })
    //   }
    // }
    ...
    // let itemsQuery
    // if (clauses.length === 0) {
    //   // No items clauses; apply no matching criteria:
    //   itemsQuery = { match_all: {} }
    // } else if (clauses.length === 1) {
    //   // A single clause? That single clause is our query:
    //   itemsQuery = clauses[0]
    // } else {
    //   // Otherwise: Group all items clauses with a boolean AND:
    //   itemsQuery = {
    //     bool: {
    //       must: clauses
    //     }
  ...
  const itemsFilterContext = (options) => {
    filterHandlers = {
      volume: (volume) => {
        const reg = /^(\d+)(\s*-\s*(\d+))?$/
        const match = volume.match(reg)
        if (!match) return null
        if (!match[1]) return null
        let volRange = {
          gte: match[1]
        }
        if (match[3]) volRange.lte = match[3]
        return {
          range: {
            'items.volumeRange': volRange
          }
        }
      },
      date: (date) => {
    ...
    const filters = Object.entries(options.query).map(([filter, value]) =>
      ((value && filterHandlers[filter]) || () => null)[value]
    ).filter(x => x)
