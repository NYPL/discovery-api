  ...
  app.resources.findByUri = function (params, opts = {}, request) {
    ...
    return scsbStatusLookup
      .then((recapBarcodesByStatus) => {
        ...
        const allItemsNoFilters = params.all_items && !paramsIncludesItemLevelFiltering
        const allItemsWithItemLevelFilters = params.all_items && paramsIncludesItemLevelFiltering
        if (allItemsNoFilters) {
          console.log('all items no filters')
          body._source.excludes = EXCLUDE_FIELDS.filter((field) => field !== '*_sort')
        } else {
          const size = allItemsWithItemLevelFilters ? MAX_ITEMS : params.items_size
