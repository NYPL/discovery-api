...
module.exports = function (app, _private = null) {
  ...
  */
  const aggregationQueriesForParams = (params) => {
    ...
    // If the search query consists of only a single filter (or a couple
    // filters of same type), just do a normal self-filtering aggregation
    // for all properties:
    if (!params.q && Object.keys(params.filters).length === 1) {
      return [buildElasticAggregationsBody(params, Object.keys(AGGREGATIONS_SPEC))]
    }
