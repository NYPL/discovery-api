...
const { AGGREGATIONS_SPEC, AGGREGATION_PACKED_FIELDS } = require('./elasticsearch/config')
...
module.exports = function (app, _private = null) {
  ...
  app.resources.findByUri = async function (params, opts = {}, request) {
    ...
    if (!resp || !resp.hits) {
      ...
      try {
        resp = await massagedResponse.massagedResponse(request, { recapBarcodesByStatus })
      } catch (e) {
  ...
  app.resources.deliveryLocationsByBarcode = async function (params, opts) {
    ...
    const resp = await Promise.all([fetchItems, lookupPatronType(params)])
    ...
    try {
      items = await Promise.all(items.map(async (item) => Item.withDeliveryLocationsByBarcode(item, scholarRoom)))
    } catch (e) {
  ...
  app.resources.search = async function (params, opts, request) {
    ...
    if (originalResp && originalResp.aggregations) {
      const serializationOpts = Object.assign({}, opts, {
        packed_fields: AGGREGATION_PACKED_FIELDS
      })
      const aggs = await AggregationsSerializer.serialize(originalResp, serializationOpts)
      resp.aggregations = aggs.itemListElement
    }
    ...
    if (params._parsedCql) {
      resp.debug.parsed = params._parsedCql
    }
