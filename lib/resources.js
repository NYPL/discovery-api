...
module.exports = function (app, _private = null) {
  ...
  app.resources.findByUri = function (params, opts, request) {
    ...
    return app.esClient.search({
      ...
      // if response has resp.hits.hits[0].inner_hits.items.hits.hits, add each item from that last hits to an array called items to the object that holds all the bib props
