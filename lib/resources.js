...
const RESOURCES_INDEX = process.env.RESOURCES_INDEX || config.get('elasticsearch').indexes.resources
  ...
  app.resources.search = function (params) {
    ...
    return app.esClient.search({
      ...
      return getItemsAvailabilityStatus(barcodes)
        ...
        .catch((error) => {
          app.logger.error('Error occurred while setting availability - ', error)
