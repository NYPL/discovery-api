...
module.exports = function (app, _private = null) {
  ...
  app.resources.findByUri = function (params, opts = {}, request) {
    ...
    return scsbClient.getBarcodesByStatusForBnum(params.uri)
      .catch((e) => {
        ...
        // Retrieve item aggregations regardless of
        // params.include_item_aggregations
        ...
        .then((resp) => {
          if (
            deepValue(resp, 'hits.hits[0].inner_hits.items.hits.hits') &&
            deepValue(resp, 'hits.hits[0]._source')
          ) {
   ...
   *  but override the indexed status for ReCAP items, whose status has been
   *  retrieved from SCSB.
   *
   ...
   *  the known unavailable items to override item statuses whose status may
   *  not be correctly indexed (because ReCAP items that are unavailable will
   *  be indexed as Available)
   */
  const itemStatusFilterWithUnavailableRecapItems = (statuses, unavailableRecapBarcodes) => {
    ...
    // Determine if all possible ReCAP statuses were selected:
    // If filtering on item status and unavailable_recap_barcodes provided
    // Add a script filter to remove matches with unwanted statuses:
    ...
    // Item's indexed status matches filter:
    const itemHasIndexedStatusClause = {
      term: { 'items.status.id': selectedRecapStatus }
    }
    ...
    // We want to build an array of clauses depending on the filter:
    const conditionsForMatchingItemStatus = []

    // If looking for Available items:
    if (selectedRecapStatus === 'status:a') {
      // Either:
      // 1) Item is on-site and Available:
      conditionsForMatchingItemStatus.push({
        bool: {
          must: [
      ...
      // ..Or 2) item is off-site and _not_ marked unavailable
      conditionsForMatchingItemStatus.push({
        bool: {
          must: [
            // Item is off-site:
      ...
      // If looking for Not Available items, either:
      // 1) Item is marked not available:
      conditionsForMatchingItemStatus.push(itemHasIndexedStatusClause)
      // Or, 2) Item is in ReCAP and among noted unavailable ReCAP items:
      conditionsForMatchingItemStatus.push({
        bool: {
          must: [
            itemIsRecapClause,
    ...
    // Final clause should be a boolean OR over the conditions:
    return { bool: { should: conditionsForMatchingItemStatus } }
  }
  ...
  const itemsQueryContext = (options) => {
    return options.merge_checkin_card_items
      ? { must: { match_all: {} } }
