...
const { parseParams, deepValue } = require('./util')
  ...
  item_location: {
    ...
    aggs: {
      _nested: {
        terms: {
          size: 100,
          field: 'items.buildingLocation.id'
        }
      }
    }
  },
...
module.exports = function (app, _private = null) {
  ...
  app.resources.deliveryLocationsByBarcode = function (params, opts) {
    ...
    const fetchItems = itemsByFilter(
      ...
      { _source: ['uri', 'type', 'items.uri', 'items.type', 'items.identifier', 'items.holdingLocation', 'items.status', 'items.catalogItemType', 'items.accessMessage', 'items.recapCustomerCode', 'items.m2CustomerCode'] }
  ...
  const itemsFilterContext = (options) => {
    ...
    const filterHandlers = {
      ...
      location: (locations) => {
        return {
          terms: {
            'items.buildingLocation.id': locations
          }
        }
      },
      status: (statuses) => {
