...
const filterClausesForDateParams = function (filters) {
  ...
  if (!hasDateAfter && !hasDateBefore) {
    return []
  } else if (hasDateAfter && hasDateBefore) {
    /**
     * When dateAfter and dateBefore used together, we want to match on:
     *   dateStartYear != null && (
  ...
  } else if (hasDateAfter) {
    /**
     * When dateAfter used alone, we want to match on:
     *   dateStartYear != null && (
     *     dateStartYear >= filters.dateAfter ||
     *     dateEndYear >= filters.dateAfter
     *   )
  ...
  } else if (hasDateBefore) {
    /**
     * When dateBefore used alone, we only need to match on:
     *   dateStartYear <= filters.dateAfter
     */
    return [{
      clause: {
