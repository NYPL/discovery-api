...
module.exports = function (app) {
  ...
  var buildElasticQuery = function (params) {
    ...
    if (params.filters) {
      ;['owner', 'subject', 'type', 'contributor', 'identifier', 'collection', 'parent'].forEach(function (param) {
  ...
  var getResourceById = function (id, cb) {
    ...
    */
    ...
    app.esClient.get({
      ...
      cb(ResourceSerializer.serialize(resp._source, {root: true}))
    }, function (err) {
      console.trace(err.message)
      cb(false)
    })
  ...
  var aggregationsByFilters = function (params, cb) {
    var query = buildElasticQuery(params)
    var aggs = {
      resourceType: { terms: { field: 'resourceType' } },
