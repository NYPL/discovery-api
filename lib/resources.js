...
module.exports = function (app) {
  ...
  app.resources.findByUri = function (params, cb) {
    ...
    return app.esClient.search({
      ...
      log.info('PROFILE: ES findByUri query took ' + (new Date() - _startTime) + 'ms')
      _startTime = new Date()
      return r
    }).then((resp) => ResourceSerializer.serialize(resp.hits.hits[0]._source, { root: true }))
      .then((r) => {
        log.info('PROFILE: serializing a findByUri result took ' + (new Date() - _startTime) + 'ms')
        return r
      })
  }
  ...
  app.resources.search = function (params) {
    ...
    log.info('Resources#search', RESOURCES_INDEX, JSON.stringify(body, null, 2))
    return app.esClient.search({
      index: RESOURCES_INDEX,
      body: body
    }).catch((e) => {
      log.error('PROFILE: Detected ES Error: ', e)
      throw e
    }).then((r) => {
      log.info('PROFILE: ES search query took ' + (new Date() - _startTime) + 'ms')
      _startTime = new Date()
      return r
    }).then((resp) => ResourceResultsSerializer.serialize(resp))
      .then((r) => {
        log.info('PROFILE: serializing a search result took ' + (new Date() - _startTime) + 'ms')
        return r
      })
  }
  ...
  app.resources.aggregations = function (params) {
    ...
    return app.esClient.search({
      ...
      log.info('PROFILE: ES agg query took ' + (new Date() - _startTime) + 'ms')
      _startTime = new Date()
      return r
    }).then((resp) => AggregationsSerializer.serialize(resp, serializationOpts))
      .then((r) => {
        log.info('PROFILE: serializing an agg result took ' + (new Date() - _startTime) + 'ms')
        return r
      })
  }
