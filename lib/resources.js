...
module.exports = function (app, _private = null) {
      ...
      .then((recapBarcodesByStatus) => {
        ...
        if (params.itemUri) {
          ...
          const itemsOptions = {
            ...
            removeElectronicResources: true,
            query: {
  ...
  const itemsQueryContext = (options) => {
    if (options.merge_checkin_card_items && options.removeElectronicResources) {
      return { must_not: { exists: { field: 'items.electronicLocator' } } }
    }
    ...
    return { must_not: { term: { 'items.type': 'nypl:CheckinCardItem' } } }
  }
