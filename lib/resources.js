...
module.exports = function (app, _private = null) {
  ...
  app.resources.findByUri = function (params, opts, request) {
    ...
    const addElectronicResources = true
    const size = params.items_size
    ...
    return app.esClient.search({
      ...
      if (resp && resp.hits && resp.hits.hits && resp.hits.hits[0] &&
        ...
        resp.hits.hits[0]._source.electronicResources = resp.hits.hits[0].inner_hits.electronicResources.hits.hits.map((resource) => resource._source)
      }
