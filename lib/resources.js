...
module.exports = function (app, _private = null) {
  ...
  const getLabelForField = (field, value) => {
    const label = app.vocabulary_labels[`${field}s`]?.[value] ?? value
    return label
  }

  /**
   * Attach provided parameters to response with labels from vocabulary map
   */
  const attachParamsWithLabels = (resp, params) => {
    const transformedFilters = {}

    for (const [field, values] of Object.entries(params.filters || {})) {
      if (Array.isArray(values)) {
        transformedFilters[field] = values.map((val) => ({
          value: val,
          label: getLabelForField(field, val)
        }))
      } else {
        transformedFilters[field] = values
      }
    }
    resp.query = { q: params.q, filters: transformedFilters }
    ...
    return app.esClient.search(body)
      .then((resp) => {
        ...
        return massagedResponse.massagedResponse(request)
          ...
          .then((resp) => attachParamsWithLabels(resp, params))
      })
      .catch((e) => {
