...
const SEARCH_SCOPES = {
  all: {
    fields: ['title.folded^10', 'description^5', 'subjectLiteral^5', 'creatorLiteral^5', 'contributorLiteral', 'note', 'shelfMark']
  },
  title: {
    fields: ['title.folded']
  },
...
module.exports = function (app) {
  ...
  app.resources.search = function (params) {
    ...
    return app.esClient.search({
      ...
      // This can be a Set. To ensure uniqueness.
        ...
        for (let item of hit._source.items) {
          ...
          if (item.identifier) {
            for (let identifier of item.identifier) {
              ...
              if (barcode_array.length === 3 && barcode_array[1] === 'barcode') { barcodes.push(barcode_array[barcode_array.length - 1]) }
            }
          }
        }
      }
      return getItemsAvailabilityStatus(barcodes)
        .then((itemsStatus) => {
        ...
        .then((barcodesAndAvailability) => {
                  ...
                  if (barcode_array.length === 3 && barcode_array[1] === 'barcode') {
                    if (barcodesAndAvailability[barcode_array[barcode_array.length - 1]] !== "Item Barcode doesn't exist in SCSB database.") {
                      ...
                      if (barcodesAndAvailability[barcode_array[barcode_array.length - 1]] === 'Available') {
                        item.status[0].id = 'status:a'
                        item.status[0].label = 'Available'
                      } else {
                        item.status[0].id = 'status:u'
                        item.status[0].label = 'Temporarily unavailable'
                      }
                    }
                  }
                }
          ...
          // call SCSB's availabity API
          // manipulate resp (the response from ElasticSearch) based on the response from SCSB
          //
          return ResourceResultsSerializer.serialize(resp)
        })
        .catch((error) => {
          log.error('Error occurred while setting availability - ', error)
          return Promise.reject(error)
        })
    })
  }

  var getItemsAvailabilityStatus = (listBarcodes) => {
    return new Promise((resolve, reject) => {
      var bodyToSend = {}
      bodyToSend = {barcodes: listBarcodes}
      var options = {
        url: config.get('recap.itemAvailabilityStatus.url'),
        headers: {
          'Accept': 'application/json',
          'api_key': config.get('recap.itemAvailabilityStatus.api-key'),
