...
module.exports = function (app, _private = null) {
  ...
  app.resources.findByUri = function (params, opts, request) {
    ...
    const addElectronicResources = params.addElectronicResources
    const size = addElectronicResources ? params.items_size : params.items_size + 1

    body = params.itemUri ? addItemQuery(body, params.itemUri) : addInnerHits(body, { size, from: params.items_from, addElectronicResources })
    ...
    return app.esClient.search({
      ...
      if (resp && resp.hits && resp.hits.hits && resp.hits.hits[0] &&
        ...
        const allItems = resp.hits.hits.[0].inner_hits.items.hits.hits.map((item) => item._source)
        const electronicResources = allItems.filter(item => item.electronicLocator)
        const physicalItems = allItems.filter(item => !item.electronicLocator).slice(0, params.items_size)
        resp.hits.hits[0]._source.items = physicalItems
        resp.hits.hits[0]._source.electronicResources = electronicResources
      }
  ...
  const addInnerHits = (body, _options = {}) => {
    ...
    const query = options.addElectronicResources ?
      { match_all: {} } :
      {
         bool: {
            should: [
              { match_all: {} },
              { exists: { field: 'electronicLocator' } }
            ]
        }
      }
...
const queryForIdentifierNumber = function (params) {
  if (params.lccn) {
    ...
    return { query: { term: { idIssn: params.issn } } }
  } else if (params.isbn) {
    return { query: { term: { idIsbn: params.isbn } } }
  } else if (params.oclc) {
    return {
      query: {
        regexp: {
          'identifierV2.value': {
            value: `.*\\(OCoLC\\)${regexEscape(params.oclc)}([^\\d].*)?`
          }
        }
      }
    }
