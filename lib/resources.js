...
var AdjacentResourcesSerializer = require('../serializers/resource/es').AdjacentResourcesSerializer
...
module.exports = function (app) {
    ...
    if (params.q && isQueryStringQuery(params.q)) {
      return new Promise((resolve, reject) => {
        app.esClient.indices.validateQuery({ q: params.q }, function (err, res) {
          if (res.valid) resolve(esSearchBody(body))
          else if (err) reject(err)
          else reject({message: 'Invalid query', query: params.q})
        })
      })
    } else {
      return esSearchBody(body) // searchResourcesByFilters(params)
    }
  }

  app.resources.adjacents = function (params) {
    params = util.parseParams(params, {
      id: { type: 'int' },
      value: { type: 'string' },
      field: { type: 'string', default: 'idLcc', allowed: ['idLcc'] },
      per_page: { type: 'int', default: 20, range: [0, 100] }
    })

    params.per_page /= 2

    var queryAdjacents = function (sortVal) {
      var queries = []

      var body = esSearchBodyForParams(params)
      // var sortVal = (new LocParser()).returnNormLcCall(value)
      body.query = { range: {} }
      body.sort = [{}]

      // Previous entries:
      body.query.range[`${params.field}Sort`] = { lt: sortVal }
      body.sort[0][`${params.field}Sort`] = { order: 'desc' }
      body.sort[0].contributorLabels = { order: 'desc' }
      queries.push(esSearchBody(body, null))

      // Later entries:
      body = Object.assign({}, body)
      body.query.range[`${params.field}Sort`] = { gte: sortVal }
      body.sort[0][`${params.field}Sort`] = { order: 'asc' }
      queries.push(esSearchBody(body, null))

      // Run two queries in parallel and join them:
      return Promise.all(queries).then(AdjacentResourcesSerializer.serialize)
    }

    // If resource id (uri) given, first look it up to get lcc:
    if (params.id) {
      return getResourceById({ id: params.id }).then((resp) => {
        // var value = resp._source.identifier.filter((id) => id.match(/^urn:lcc:/))[0]
        console.log('sort by ', resp._source.idLccSort)
        return queryAdjacents(resp._source.idLccSort)
      })

    // Otherwise, just look up adjacents by given lcc:
    } else {
      var sortVal = (new LocParser()).returnNormLcCall(params.value)
      return queryAdjacents(sortVal)
    }
  }
  ...
  app.resources.findByOwi = function (params) {
    var body = esSearchBodyForParams({ filters: { identifier: 'urn:owi:' + params.value } })
    return esSearchBody(body)
  }
  ...
  var esSearchBody = function (body, serializer) {
    if ((typeof serializer) === 'undefined') serializer = ResourceResultsSerializer.serialize
    ...
    return new Promise((resolve, reject) => {
      app.esClient.search({
        ...
        if (serializer) resolve(serializer(resp))
        else resolve(resp)
      }, function (err) {
        reject(err.message)
      })
  ...
  var esSearchBodyForParams = (params) => {
    ...
    var body = {
      from: params.page ? (params.per_page * (params.page - 1)) : 0,
      size: params.per_page
    }

    var hasSignificantQuery = Object.keys(query).length > 0
    // console.log('has sig query: ', hasSignificantQuery)
    if (hasSignificantQuery) {
      body.query = {
        function_score: {
          query: query,
          field_value_factor: {
