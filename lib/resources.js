...
const queryHasParams = function (params) {
  return params.q || ApiRequest.ADVANCED_SEARCH_PARAMS.some((param) => params[param])
}
...
const buildElasticBody = function (params) {
  ...
  if (params.filters || queryHasParams(params)) {
    const query = buildElasticQuery(params)
...
const buildElasticQueryForKeywords = function (params) {
  ...
  if (request.hasScope(['standard_number', 'callnumber'])) {
    // If matching on callnumber/standard-number, allow prefix matches on callnumber:
    should.push({
      nested: {
        path: 'items',
        query: {
          prefix: {
            'items.shelfMark.raw': namedQuery({
              value: request.querySansQuotes()
            }, 'prefix items.shelfMark.raw')
          }
        }
      }
    })
  }
  if (request.hasScope(['standard_number'])) {
    // If matching on standard-number, require full/prefix match on known identifiers:
    should = should.concat([
      ...
      },
      {
        nested: {
          ...
          query: {
            term: {
              'items.idBarcode': namedQuery({
                value: request.querySansQuotes()
              }, 'term items.idBarcode')
            }
          }
        }
      },
  ...
  // For search_scope all, contributor, subject, and title, build multi_match query on known fields:
  if (SEARCH_SCOPES[params.search_scope].fields) {
...
const buildElasticQuery = function (params) {
  ...
  if (request.hasKeyword()) {
    ...
    if (queryStringMatcher) {
      query.addMust(queryStringMatcher)
    }

    const shoulds = buildBoostedShouldClauses(request)
    query.addShoulds(shoulds)
  }

  if (request.advancedSearchParamsThatAreAlsoScopes()) {
    const advancedParamQueries = request
      .advancedSearchParamsThatAreAlsoScopes()
      .map((param) =>
        buildElasticQueryForKeywords({ q: params[param], search_scope: param })
      )

    query.addMusts(advancedParamQueries)
  }

  const keywordFilter = buildFilterForKeyword(request)
  if (keywordFilter) {
    query.addFilter(keywordFilter)
  }

  if (params.filters) {
    // Merge filter-specific ES query into the query we're building:
    const userFilters = buildElasticQueryForFilters(params)
    if (userFilters.length) {
      query.addFilters(userFilters)
    }
  }
...
const buildBoostedShouldClauses = (request) => {
  let shoulds = []
  ...
  */
  ...
  if (request.hasScope(['all', 'title', 'contributor'])) {
    // Boost popular bibs:
    Object.entries(POPULARITY_BOOSTS).forEach(({ gte, boost }) => {
      shoulds.push({ range: { popularity: namedQuery({ gte, boost }, `popularity >= ${gte}`) } })
    })
  }

  if (request.hasScope(['all', 'title'])) {
    shoulds = shoulds.concat([
      {
        match_phrase: {
          'title.folded': namedQuery({
            query: request.querySansQuotes(),
            boost: 25
  ...
  if (request.hasScope(['contributor'])) {
    shoulds = shoulds.concat([
      {
        prefix: {
  ...
  if (request.hasScope(['all', 'contributor'])) {
    shoulds = shoulds.concat([
      {
        prefix: {
          'creatorLiteralNormalized.keywordLowercased': namedQuery({
            value: request.querySansQuotes(),
            boost: 100
          }, 'prefix creatorLiteralNormalized')
        }
      }
    ])
  }

  if (request.hasScope(['all', 'standard_number', 'callnumber'])) {
    shoulds.push({
      nested: {
        path: 'items',
        query: {
  ...
  if (request.hasScope(['callnumber'])) {
    shoulds.push({
      nested: {
        path: 'items',
        query: {
  ...
  if (request.hasScope(['standard_number'])) {
            ...
            should: [
              {
                ...
                term: {
                  'items.identifierV2.value': namedQuery({ value: request.querySansQuotes() }, 'term identifierV2.value')
                }
              },
              {
                prefix: {
                  'items.shelfMark.raw': namedQuery({ value: request.querySansQuotes() }, 'prefix shelfMark.raw')
                }
              }
            ]
          }
    ...
    if (/\d{6,}/.test(request.querySansQuotes())) {
      shoulds.push({
        prefix: {
          idIsbn_clean: namedQuery({
            value: request.querySansQuotes()
...
const buildFilterForKeyword = (request) => {
  if (!request.queryIsFullyQuoted()) return null

  // When query is fully quoted, add filter to restrict to strong phrase matches:
  const phraseMatchingClauses = []

  if (request.hasScope(['all', 'title'])) {
    phraseMatchingClauses.push({
      match_phrase: {
        'titleDisplay.folded': namedQuery({
          query: request.querySansQuotes()
        }, 'match_phrase title.folded')
      }
    })
  ...
  return { bool: { should: phraseMatchingClauses } }
}
