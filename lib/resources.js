...
module.exports = function (app, _private = null) {
  ...
  const addInnerHits = (body) => {
    const field = body.query.function_score.query.bool.should ? 'should' : 'must'
    body.query.function_score.query.bool[field].push({
      nested: {
        path: 'items',
        query: { 'match_all': {} },
        inner_hits: {
  ...
  app.resources.search = function (params, opts, request) {
    ...
    const shouldAddInnerHits = body.query &&
      ...
      (body.query.function_score.query.bool.should || body.query.function_score.query.bool.must)
...
const buildElasticQuery = function (params) {
  ...
  if (advancedParamQueries.length) {
    ...
    query.bool.must = (query.bool.should || []).concat(advancedParamQueries.reduce((acc, el) =>
      acc.concat(el)
    ))
