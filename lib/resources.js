...
module.exports = function (app) {
  ...
  app.resources.findById = function (params, cb) {
    ...
    // searchResourcesByFilters({ uri: id, per_page: 1 }, cb)
    ...
    app.db.returnCollectionTripleStore('resources', function (err, resources) {
      ...
      resources.find({uri: parseInt(id)}).toArray(function (err, results) {
        ...
        if (results.length === 0) {
          ...
          cb(ResourceSerializer.serialize(results[0]))
          // cb(results[0])
        }
      })
    })
  ...
  var getResourceById = function (id, cb) {
    // Running search because we need the _type to use the id, so instead we just search by uri
    app.esClient.search({
      index: 'resources',
      body: {
        query: {
      ...
      // Get the single document from the hits
      var res = resp.hits
      if (res && res.hits && res.hits.length > 0) res = res.hits[0]
      if (res && res._source) res = res._source
      cb(ResourceSerializer.serialize(res))
    }, function (err) {
      console.trace(err.message)
      cb(false)
    })

    // using get endpoint only works if you know the _type...
    /*
    app.esClient.get({
      index: 'resources',
      type: 'collectionitem',
      id: id
    }).then((resp) => cb(ResourceSerializer.serialize(resp)), function (err) {
      console.trace(err.message)
      cb(false)
    })
    */
  ...
  var aggregationsByFilters = function (params, cb) {
    ...
    var serializationOpts = {
      packed_fields: ['subject', 'contributor']
    }
    app.esClient.search({
      index: 'resources',
      body: {
