...
module.exports = function (app, _private = null) {
  ...
  const esRangeValue = (range) => {
    ...
    if (range.length === 2) {
      // if the two range values are the same, query up until the next year
      if (range[0] === range[1]) {
        dateRangeQuery.lt = range[0] + 1
      } else {
        // otherwise search on both range values
        dateRangeQuery.lte = range[range.length - 1]
      }
  ...
  const itemsFilterContext = (options) => {
    ...
    const filterHandlers = {
      volume: (volume) => {
        return {
          range: {
            'items.volumeRange': esRangeValue(volume)
          }
        }
      },
      date: (date) => {
        return {
          range: {
            'items.dateRange': esRangeValue(date)
          }
        }
      },
      format: (format) => {
      ...
      location: (location) => {
        return {
          terms: {
            'items.holdingLocation.id': location
          }
        }
      },
      status: (status) => {
