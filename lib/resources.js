...
const SEARCH_SCOPES = {
  all: {
    fields: ['title.folded^5', 'description.folded', 'subjectLiteral.folded', 'creatorLiteral.folded', 'contributorLiteral.folded', 'note.label.folded', 'publisherLiteral.folded', 'seriesStatement.folded', 'titleAlt.folded', 'titleDisplay.folded']
  },
  title: {
    fields: ['title.folded^5', 'seriesStatement.folded', 'titleAlt.folded', 'uniformTitle.folded', 'titleDisplay.folded']
  },
  contributor: {
    fields: ['creatorLiteral.folded^2', 'contributorLiteral.folded']
  },
  subject: {
    fields: ['subjectLiteral.folded']
  },
  series: {
...
const EXCLUDE_FIELDS = ['uris', '*_packed', '*_sort', 'items.*_packed']
...
const escapeQuery = function (str) {
  ...
  const unrecognizedFieldQueryRegex = new RegExp(`(^|\\s)(?!(${allowedFields.join('|')}))\\w+(:)`, 'g')
  str = str.replace(unrecognizedFieldQueryRegex, (match) => {
    return match.replace(/:/, '\\:')
  })

  // TODO should also do something about these lucene chars:
  // '( ) { } [ ] :
  return str
}
...
var buildElasticQuery = function (params) {
  ...
  // const hasQueryStringQuery = params.q && /\w{3,}:[\w"\[<>]| (AND|OR) |"(\w+\s?)+"/.test(params.q)
