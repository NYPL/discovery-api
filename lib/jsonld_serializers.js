...
const parentLocations = require('../data/parentLocations.json')
...
class ResourceSerializer extends JsonLdItemSerializer {
  ...
  async statements () {
    ...
    if (this.body.collectionIds) {
      stmts.collection = ResourceSerializer.getFormattedCollection(this.body.collectionId)
...
ResourceSerializer.getFormattedCollection = function (collectionId) {
  const prefLabel = nyplCore.collections()[collectionId]?.label
  const parentLocation = parentLocations.find((loc) => loc.value === collectionId.slice(0, 2))?.label
  if (!prefLabel) return null
  return [{
    '@id': collectionId,
    prefLabel,
