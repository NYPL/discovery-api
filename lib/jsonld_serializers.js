...
class ResourceSerializer extends JsonLdItemSerializer {
  ...
  statements () {
    ...
    if (this.body.numItems) stmts.numItems = this.body.numItems
    if (this.body.numAvailable) stmts.numAvailable = this.body.numAvailable
  ...
  statements () {
    ...
    try {
      stmts.values = values.map((v) => {
        ...
        else if (this.options.packed_fields && this.options.packed_fields.indexOf(field) >= 0) {
