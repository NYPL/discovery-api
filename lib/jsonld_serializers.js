  ...
  statements () {
    ...
    Object.keys(body).forEach((field) => {
      ...
      if ((match = field.match(/^contributor_(\w+)$/))) {
        ...
        body[field].forEach((contributor) => {
          var ind = -1
          // Contributor already there? (should be)
          if ((ind = R.findIndex(R.propEq('id', contributor.id))(body.contributor)) >= 0) {
            // Ensure has roles property:
            if (!body.contributor[ind].roles) body.contributor[ind].roles = []
            // Add the role:
            body.contributor[ind].roles.push(roleStatement)
          } else {
            // Contributor not in array for some reason; add it with role:
            body.contributor.push(Object.assign(contributor, {roles: [roleStatement]}))
          }
        })
        delete body[field]
      }
    })
    return Object.assign({}, {'@id': this.resultId()}, body)
  }
...
class ResourceResultsSerializer extends SearchResultsSerializer {
  ...
  static serialize (resp) {
    ...
    hits.forEach(function (h) {
      ...
      var serialized = ResourceSerializer.serialize(h._source)
