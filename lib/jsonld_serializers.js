...
ResourceSerializer.formatItemFilterAggregations = function (aggregations) {
  return Object.keys(aggregations).map((aggregation) => {
    const field = aggregation.slice(5)
    const values = aggregations[aggregation]._nested.buckets.map((bucket) => {
      const isFormatLiteral = field === 'format'
      const [value, label] = bucket.key.split('||')
      return {
        value: isFormatLiteral ? bucket.key : value,
        count: bucket.doc_count,
        label: isFormatLiteral ? bucket.key : label
      }
