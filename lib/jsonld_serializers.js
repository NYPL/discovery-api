...
class ResourceSerializer extends JsonLdItemSerializer {
  ...
  async statements () {
    ...
    if (this.body.collectionIds) {
      stmts.collection = ResourceSerializer.getFormattedCollections(this.body.collectionIds)
      delete stmts.collectionIds
    }
...
ResourceSerializer.getFormattedCollections = function (collectionIds) {
  return collectionIds.map(collectionId => ResourceSerializer.formatCollection(collectionId))
}
...
class ItemResourceSerializer extends JsonLdItemSerializer {
  ...
  static getFormattedHoldingLocation (location) {
    ...
    // Add collection access type from corresponding Core location
    const collectionAccessType =
      nyplCore.sierraLocations()[locationId]?.collectionAccessType
    return [{
      '@id': loc['@id'],
...
class AggregationsSerializer extends JsonLdListSerializer {
  ...
  static async serialize (resp, options) {
    ...
    const hits = resp.hits?.total ?? 0
    return (new AggregationsSerializer(items, Object.assign({ extraRootProperties: { totalResults: hits } }, options))).format()
  }
}
    ...
    try {
      stmts.values = values.map((v) => {
        if (this.options.packed_fields && this.options.packed_fields.indexOf(field) >= 0) {
          ...
          const collectionLabels = ResourceSerializer.formatCollection(v.value)
          if (collectionLabels) {
            v.label = collectionLabels.prefLabel
          } else {
            v.label = v.value
          }
        } else {
