...
class ResourceSerializer extends JsonLdItemSerializer {
  ...
  statements () {
    ...
    ; ['dimensions', 'extent', 'publisher', 'placeOfPublication'].forEach((property) => {
    ...
    if (this.body.items) {
      stmts.items = this.body.items.map((item) => {
        ...
        if (item.identifier) {
          subdoc.identifier = item.identifier
        }
        if (item.owner_packed) {
          subdoc.owner = JsonLdItemSerializer.parsePackedStatement(item.owner_packed)
        }
        if (item.shelfMark) {
          subdoc.shelfMark = item.shelfMark
        }
        return subdoc
      })
    }
