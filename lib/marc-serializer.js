...
/**
 * Serializes a bib with suppressed fields
 * @param {Bib} bib - Bib to serialize
 * @returns {SerializedMarc} Serialized bib with masked varFields
 */
MarcSerializer.serialize = function (bib) {
  const suppressedVarFields = bib.varFields.map((field) => {
    // pass LDR through
    ...
    if (matchingRule.directive !== 'include') return field

    console.log(
      'MARC suppressing',
      MarcSerializer.describeField(field),
      `rule=${matchingRule.fieldTag}`
    )
    ...
    parallels.forEach((p) => {
      console.log(
        'MARC suppressing parallel 880',
        MarcSerializer.describeField(p),
        `linked to ${MarcSerializer.describeField(field)}`
      )
