...
MarcSerializer.orderedFieldTags = arrayUnique(MarcSerializer.mappingRules.map((rule) => rule.fieldTag))

/**
 * Given the raw source of a webpub.def file, returns an array of usable
 * rules that relate field labels to marc queries.
 */
MarcSerializer.parseWebpubToAnnotatedMarcRules = function (webpubContent) {
 ...
 */
MarcSerializer.buildAnnotatedMarcRules = function (webpubContent) {
  ...
  return MarcSerializer.parseWebpubToAnnotatedMarcRules(webpubContent)
...
MarcSerializer.formatVarFieldMatch = function (matchingVarField, rule) {
  ...
  const source = MarcSerializer.buildSourceWithMasking(matchingVarField, rule)
 ...
 */
MarcSerializer.formatVarFieldMatches = function (matchingVarFields, rule) {
  return matchingVarFields.map((field) => MarcSerializer.formatVarFieldMatch(field, rule))
}
 ...
 *
 */
MarcSerializer.addStatementsForVarFieldForRule = function (doc, bib, varField, rule) {
  const content = MarcSerializer.formatVarFieldMatch(varField, rule)
  //
  if (content) doc = MarcSerializer.addStatementsToDoc(doc, rule, [content])
  ...
  if (parallelNumbers.length > 0) {
    ...
    const matchingParallels = MarcSerializer.matchingMarcFields(bib, Object.assign({}, rule, { fieldTag: 'y', marcIndicatorRegExp: /^880/ }))
      .map((varField) => {
        return {
          field: varField,
    ...
    if (matchingParallels.length > 0) {
      ...
      const parallelContent = MarcSerializer.formatVarFieldMatches(matchingParallels, rule)
      const pseudoRule = { label: parallelLabel, fieldTag: varField.fieldTag }
      doc = MarcSerializer.addStatementsToDoc(doc, pseudoRule, parallelContent)
    }
  }
...
MarcSerializer.initialStateObjectForSerialization = function () {
  return MarcSerializer.orderedFieldTags.reduce(function (acc, tag) {
    acc[tag] = []
    return acc
  }, {})
}
    ...
    MarcSerializer.mappingRules.forEach((rule) => {
      if (!foundMatch && MarcSerializer.varFieldMatches(field, rule)) {
        if (rule.directive === 'include') {
          doc = MarcSerializer.addStatementsForVarFieldForRule(doc, bib, field, rule)
        }
        foundMatch = true
      }
    })
