...
exports.apiEntrypointDoc = {
  version: pjson.version,
  '@context': {
    hydra: 'http://www.w3.org/ns/hydra/core#',
    vocab: 'http://api.data.nypl.org/doc/#',
    EntryPoint: 'vocab:EntryPoint',
    resources: {
      '@id': 'vocab:EntryPoint/resources',
      '@type': '@id'
    }
  },
  resources: '/api/v1/resources/search/'
}
...
exports.apiDoc = {
  ...
  '@id': 'http://api.data.nypl.org/doc/',
  title: 'data.nypl.org API',
  entrypoint: 'http://api.data.nypl.org',
  supportedClass: [
    {
      '@id': 'vocab:Resource',
      '@type': 'hydra:Class',
      subClassOf: null,
      label: 'Resource',
      description: 'A collection, component, item, or capture.',
      supportedOperation: [
        {
          '@id': '_:resource_retrieve',
          '@type': 'hydra:Operation',
          method: 'GET',
          label: 'Retrieves a Resource entity',
          description: null,
          expects: null,
          returns: 'vocab:Resource',
          statusCodes: [
            {
              code: 404,
      ...
      supportedProperty: [
        {
          property: {
            '@id': 'vocab:Resource/title',
            '@type': 'rdf:Property',
            label: 'title',
            description: 'The resource\'s title',
            domain: 'vocab:Resource',
            range: 'http://www.w3.org/2001/XMLSchema#string',
            supportedOperation: []
          },
          'hydra:title': 'title',
          'hydra:description': 'The resource\'s title',
          required: null,
          readonly: true,
          writeonly: false
        },
...
exports.parseParam = function (val, spec) {
  ...
  if (spec.allowed) {
    if (spec.allowed.indexOf(val) < 0) return spec.default
  }
...
exports.gatherParams = function (req, acceptedParams) {
  ...
  if (req.params.id) params.value = req.params.id
  return params
}
