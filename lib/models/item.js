...
class Item {
  constructor (innerHitItem) {
    ...
    this.itemIsInRecapMissingRecapCustomerCode = this.itemIsInRecap && !this.recapCustomerCode
  ...
  get holdingLocation () {
    if (!Array.isArray(this.item.holdingLocation)) {
      ...
      if (/^i/.test(this.item.uri)) {
        logger.warn(`DeliveryLocationsResolver#extractLocationCode: Item missing holdingLocation: ${this.item.uri}`)
      }
      return false
    }
  ...
  get eddRequestable () {
    let eddRequestable
    if (this.item.electronicLocator) return false
    if (!this.recapCustomerCode) {
      eddRequestable = DeliveryLocationsResolver.eddRequestableByOnSiteCriteria
    } if (this.itemIsInRecapMissingRecapCustomerCode) {
      const requestableBasedOnHoldingLocation = this.nyplItem ? this.location.requestableBasedOnHoldingLocation : true
      eddRequestable = requestableBasedOnHoldingLocation
    } else if (this.recapCustomerCode && (!this.nyplItem || this.location.requestableBasedOnHoldingLocation)) {
      eddRequestable = DeliveryLocationsResolver.__eddRequestableByCustomerCode(this.recapCustomerCode)
    } else {
