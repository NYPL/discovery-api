...
const addNumItemsMatched = (resp, request) => {
  if (util.checkForNestedHitsAndSource(resp)) {
    ...
    const hasElectronicResources = theBib._source.numElectronicResources && theBib._source.numElectronicResources[0] > 0
    ...
    if (hasElectronicResources) {
      ...
      itemFormatQueryMatchesBibMaterialType = request.query.item_format && request.query.item_format.includes(bibMaterialType)
      ...
      requestHasItemFilters = propertiesToTest.some((param) => { return param in request.query })
      // if item_format of electronic resource item matches bib material type, we need to decrement
      const itemFormatMatchesBibAndFormatIsOnlyFilter = itemFormatQueryMatchesBibMaterialType && !requestHasItemFilters
      if (itemFormatMatchesBibAndFormatIsOnlyFilter || !requestHasItemFilters) numItemsMatched -= 1
    }
