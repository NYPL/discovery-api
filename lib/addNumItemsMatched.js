...
const addNumItemsMatched = (resp, request) => {
  if (util.checkForNestedHitsAndSource(resp)) {
    ...
    let requestHasItemFilters
    let itemFormatQueryMatchesBibMaterialType
    // let itemFormatQueryMatchesBibMaterialTypePlus
    ...
    if (hasElectronicResources) {
      ...
      if ((itemFormatQueryMatchesBibMaterialType && !requestHasItemFilters) || !requestHasItemFilters) numItemsMatched -= 1
    }
