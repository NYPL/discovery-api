...
class DeliveryLocationsResolver {
  ...
  static eddRequestableByOnSiteCriteria (item) {
    console.log('eddrequestablebyonsitecriteria')
    ...
    logger.info({ message: `EDD criteria check for ${item.uri}`, finalResult, eddCriteriaChecks })
    return finalResult
  }
  ...
  static getRecapDeliveryInfo (item) {
    ...
    if (!hasRecapCustomerCode) {
      ...
      deliveryLocation = new Location({ recapCustomerCode: item.recapCustomerCode[0] }).deliveryLocation
  ...
  static getOnsiteDeliveryInfo (item) {
    ...
    if (deliverableToResolution === 'm2-customer-code' && item.m2CustomerCode && item.m2CustomerCode[0]) {
      deliveryInfo.deliveryLocation = new Location({ m2CustomerCode: item.m2CustomerCode[0] })
    }
