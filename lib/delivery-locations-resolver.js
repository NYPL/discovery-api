...
class DeliveryLocationsResolver {
  ...
  static attachRecapDeliveryInfo (item, recapCustomerCode) {
    ...
    if (requestableBasedOnHoldingLocation(item) || !isItemNyplOwned(item)) {
      item.deliveryLocation = this.deliveryLocationsByRecapCustomerCode(recapCustomerCode)
    }
    item.eddRequestable = this.__eddRequestableByCustomerCode(recapCustomerCode)
    return item
  }
  ...
  static attachOnsiteDeliveryInfo (item) {
    ...
    // should be found by M2 code AND that the location code is in fact requestable
    if (deliverableToResolution === 'm2-customer-codes' && requestableBasedOnHoldingLocation(item)) {
      item.deliveryLocation = this.deliveryLocationsByM2CustomerCode(item.m2CustomerCode[0])
    }
    // if no value, default to sierra location lookup
    if (!deliverableToResolution) {
      item.deliveryLocation = sierraLocations[holdingLocationCode].sierraDeliveryLocations
    }
    ...
    return this.__recapCustomerCodesByBarcodes(recapBarcodes)
      .then((barcodeToRecapCustomerCode) => {
        ...
        return items.map((item) => {
          ...
          const recapCustomerCode = barcodeToRecapCustomerCode[barcode]
          // If recap has a customer code for this barcode, map it by recap cust code:
          if (recapCustomerCode) {
            item = this.attachRecapDeliveryInfo(item, recapCustomerCode)
          ...
          item.deliveryLocation = this.__formatLocations(item.deliveryLocation, deliveryLocationTypes)
