...
class RequestabilityResolver {
  ...
  static buildPhysRequestable (item) {
    ...
    const deliveryInfo = isInRecap(item)
      ? DeliveryLocationsResolver.getRecapDeliveryInfo(item)
      : DeliveryLocationsResolver.getOnsiteDeliveryInfo(item)
    const numDeliveryLocations = deliveryInfo.deliveryLocation?.length
    if (isItemNyplOwned(item) && item.specRequestable) {
      physRequestable = false
      physRequestableCriteria = 'NYPL owned special collections item'
    } else if (isItemNyplOwned(item) && !hasBarcode) {
      physRequestable = false
      physRequestableCriteria = 'NYPL item missing barcode'
    } else if (isItemNyplOwned(item) && !DeliveryLocationsResolver.requestableBasedOnHoldingLocation(item)) {
