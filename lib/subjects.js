...
const PAINLESS_ALPHA_PREFIX_SORT_ASC = `
  ...
  for (Map variant : params._source.getOrDefault('variants', [])) {
      ...
      if (variantKeyword.startsWith(searchValue)) {
          if (sortValue == null || variantKeyword.compareTo(sortValue) > 0) {
      ...
      .then((resp) => {
        return {
          ...
          subjects: resp.hits?.hits?.map((hit) => {
            ...
            if (hit.highlight || hit.inner_hits.variants.hits.hits.length === 0) {
...
const buildElasticSubjectsBody = function (params) {
  ...
  body.highlight = { fields: { 'preferredTerm.keyword_normalized': highlightDef, 'preferredTerm.keyword': highlightDef } }
