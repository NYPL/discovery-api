      ...
      .then((resp) => {
        return {
          ...
          subjects: resp.hits?.hits?.map((hit) => {
            if (hit.matched_queries?.[0] === 'preferredTerm' || hit.matched_queries?.[0] === 'preferredTermPrefix' || hit.matched_queries?.[0] === 'preferredTermMatchBoolPrefix') { // if match is on preferredTerm, use that regardless of variant matches
              return {
                '@type': 'preferredTerm',
