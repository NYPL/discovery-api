      ...
      .then((resp) => {
        return {
          ...
          subjects: resp.hits?.hits?.map((hit) => {
            if ('preferredTerm' in hit.highlight || 'preferredTerm.keyword' in hit.highlight) {
              ...
              let highlight
              if ('variants' in hit.highlight) {
                highlight = 'variants'
              } else {
                highlight = 'variants.keyword'
              }
              const matchedVariantTerm = hit.highlight[highlight][0]
              // can be lowercase or capitalized depending on keyword or text search, so find the real casing
              const properlyFormattedVariantTerm = hit._source.variants.find((formattedVariant) => {
                if (formattedVariant.toLowerCase() === matchedVariantTerm.toLowerCase()) {
                  return true
                } else return false
              })
              ...
              return {
                '@type': 'variant',
                termLabel: properlyFormattedVariantTerm,
                preferredTerms: [
                  {
                    termLabel: hit._source.preferredTerm,
                    count: hit._source.count
...
const buildElasticSubjectsBody = function (params) {
  ...
  body.highlight = { fields: { 'variants.keyword': highlightDef, variants: highlightDef, 'preferredTerm.keyword': highlightDef, preferredTerm: highlightDef } }
