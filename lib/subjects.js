...
module.exports = function (app, _private = null) {
  ...
  // Browse for subjects starting with a provided token:
    ...
    return app.esClient.search(body, process.env.SUBJECTS_INDEX)
      .then((resp) => {
        ...
        return massagedResponse.massagedResponse(request)
          .catch((e) => {
            // TODO: Return raw ES response in case of error
            return resp
          })
          .then((resp) => {
            // TODO: transform response into friendly format for front end
            return resp
          })
      })
  }
