      ...
      .then((resp) => {
        return {
          ...
          subjects: resp.hits?.hits?.map((hit) => {
            ...
            if ('preferredTerm' in hit.highlight) {
              ...
              const lowerCaseMatchedVariantTerm = hit.highlight['variants'][0]
...
const buildElasticSubjectsBody = function (params) {
  ...
  body.highlight = { fields: { 'variants.has': highlightDef, 'preferredTerm.has': highlightDef } }
