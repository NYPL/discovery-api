...
async function itemsByFilter (identifierValues, app) {
  ...
  const resp = await app.esClient.search(body)
  if (!resp || !resp.hits || resp.hits.total === 0) return Promise.reject(new Error('No matching items'))
  const respWithLabels = new LocationLabelUpdater(resp).responseWithUpdatedLabels()

  const bibs = respWithLabels.hits.hits.map((doc) => doc._source)
  bibs.forEach((bib) => {
    bib.items = bib.items.map(i => Object.assign(i, { identifier: [`urn:bnum:${bib.uri}`].concat(i.identifier) }))
  })
  const items = bibs.map(bib => bib.items).reduce((acc, el) => acc.concat(el))
...
const lookupPatronType = async function (params) {
  try {
    return await AvailableDeliveryLocationTypes.getScholarRoomByPatronId(params.patronId)
  } catch (e) {
