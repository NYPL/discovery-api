...
const nyplSourceAndId = async function (uri) {
  const nyplSourceMapper = await NyplSourceMapper.instance()
  const { id, nyplSource } = nyplSourceMapper.splitIdentifier(uri) ?? {}
  if (!id || !nyplSource) {
    throw new errors.InvalidParameterError(`Invalid bnum: ${uri}`)
  }
